...
HTML = """<!doctype html>
<html><head><meta charset="utf-8"><title>Symphony + Akida fleet</title>
<style>
  ...
  * { box-sizing:border-box; } body { margin:0; font:15px/1.5 system-ui,sans-serif; background:var(--bg); color:var(--fg); }
  ...
  .row .val { width:120px; text-align:right; font-variant-numeric:tabular-nums; }
  #msg { color:var(--muted); font-size:13px; margin:14px 0; }
  .err { color:#f87171; }
</style></head><body>
<header>
  <h1>Symphony + Akida &mdash; fleet inference</h1>
  <p>Submit a batch as one Symphony SOAM session; the session manager fans the tasks across every Akida chip in parallel.</p>
</header>
<main>
  <div class="controls">
    <div><label>Model</label><select id="model"></select></div>
    <div><label>Samples</label><input id="count" type="number" value="2000" min="1" step="100" style="width:120px"></div>
